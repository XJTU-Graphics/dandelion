...
TEST_CASE("Transformation", "[basic]")
{
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        INFO(std::format("center is {:.2f}", test_object.center));
        INFO(std::format("scaling is {:.2f}", test_object.scaling));
        INFO(
            std::format(
                "rotation is {:.2f}", Vector3f(degrees(x_rad), degrees(y_rad), degrees(z_rad))
            )
        );
        ...
        INFO(std::format("the answer is{:>5.1f}", answer));
        INFO(std::format("but it should be{:>5.1f}", reference));
...
TEST_CASE("Perspective Projection", "[basic]")
{
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        INFO(
            std::format(
                "near: {:.3f}, far: {:.1f}, fov_y: {:.1f} deg, aspect ratio: {:.2f}",
