...
bool Group::load_models(const string& file_path)
{
    ...
    for (size_t mesh_id = 0; mesh_id < n_meshes; ++mesh_id) {
        ...
        for (unsigned int vertex_id = 0; vertex_id < n_vertices; ++vertex_id) {
            ...
            object_positions.emplace_back(vertex.x, vertex.y, vertex.z);
            object_normals.emplace_back(normal.x, normal.y, normal.z);
        }
        // Load faces into the object's GL::Mesh.
        for (unsigned int face_id = 0; face_id < n_faces; ++face_id) {
            const size_t vertices_per_face = faces[face_id].mNumIndices;
            // Skip non-triangle faces.
            if (vertices_per_face > 3) {
                logger->warn("face {} in file has more than 3 vertices, skipped", face_id);
                continue;
            }
            array<unsigned int, 3> f;
            for (unsigned int current_vertex = 0; current_vertex < vertices_per_face;
                 ++current_vertex) {
                const unsigned int vertex_id = faces[face_id].mIndices[current_vertex];
                f[current_vertex]            = vertex_id;
        ...
        if (material_name != assimp_default_material_name) {
            object.material                = make_unique<PhongMaterial>();
            PhongMaterial& object_material = dynamic_cast<PhongMaterial&>(*object.material.get());
            aiColor3D      color;
            float          shininess;
        ...
        logger->info(
            "summary: {} vertices, {} edges, {} faces", mesh->mNumVertices, object_edges.size(),
            object_faces.size()
        );
        object.rebuild_BVH();
        logger->info(
...
bool Group::save_models(const string& file_path)
{
    ...
    for (size_t i = 0; i < num_objects; ++i) {
        ...
        if (object->material->type() != MaterialType::Phong) {
            logger->warn(
                "object {} does not use Phong material, only an empty material will be saved",
                object->id
            );
            continue;
        }
        const PhongMaterial& material = dynamic_cast<PhongMaterial&>(*object->material.get());
    ...
    for (size_t i = 0; i < num_objects; ++i) {
        ...
        size_t num_vertices = object->mesh.positions.size();
        size_t num_normals  = object->mesh.normals.size();
        ...
        for (size_t vertex_id = 0; vertex_id < num_vertices; ++vertex_id) {
            auto vertex                   = object->mesh.positions[vertex_id];
            ai_mesh->mVertices[vertex_id] = {vertex.x(), vertex.y(), vertex.z()};
            auto normal                   = object->mesh.normals[vertex_id];
        ...
        size_t num_faces = object->mesh.faces.size();
        ...
        for (size_t face_id = 0; face_id < num_faces; ++face_id) {
            auto face                            = object->mesh.faces[face_id];
