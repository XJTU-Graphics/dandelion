            ...
            for (auto group = scene->groups.begin(); group != scene->groups.end(); ++group) {
                ...
                for (auto object = objects.begin(); object != objects.end(); ++object) {
                    if (object->get() == selected_object) {
                        logger->info(
                            "delete object \"{}\" (ID: {}) from group \"{}\" (ID: {})",
                            selected_object->name, selected_object->id, (*group)->name, (*group)->id
                        );
                ...
                if (found) {
                    if (objects.empty()) {
                        logger->info(
                            "cascade to delete an empty group \"{}\" (ID: {})", (*group)->name,
                            (*group)->id
                        );
...
void Controller::select(SelectableType element)
{
    ...
    visit(
        overloaded{
            ...
            [this](Edge* edge) { select_edge(edge); }, [this](Face* face) { select_face(face); },
...
void Controller::on_rotating(bool initial)
{
    ...
    auto            to_trackball_space = [this](const ImVec2& mouse_pos) -> Vector3f {
        ...
        Vector3f track_pos(
            mouse_pos.x - window_width / 2.0f, -(mouse_pos.y - window_height / 2.0f), 0.0f
        );
    ...
    Quaternionf quat = Quaternionf::FromTwoVectors(
